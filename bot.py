...
# With Services, Nested Folders, Dynamic Buttons, Points System & BUY POINTS
# =========================
...
def points_cmd(m):
    ...
    points_msg += f"• 💎 **Purchase:** Click 🛒 BUY POINTS\n\n"
...
def buy_points_menu(m):
    ...
    if not active_packages:
        bot.send_message(uid, "❌ No points packages available right now. Please contact admin!")
        return
    
    message = f"🛒 **BUY POINTS** 🛒\n\n"
    message += f"💰 **Your Current Balance:** `{user.points()}` points\n\n"
    message += f"📦 **Available Packages:**\n\n"
    ...
    kb.add(InlineKeyboardButton("📞 Contact Admin to Buy", url=f"https://t.me/{admin_username}"))
...
def get_points_callback(c):
    ...
    bot.edit_message_text(
        ...
        f"• 🎁 **Referral System:** Share your link\n"
        f"• 🏆 **Redeem Codes:** Use coupon codes\n"
        f"• 💎 **Purchase:** Click 🛒 BUY POINTS\n\n"
        f"💡 **Tip:** More points = More VIP methods!",
        uid,
        c.message.message_id,
        parse_mode="Markdown"
    )
...
def check_balance_callback(c):
    ...
    bot.edit_message_text(
        ...
        f"Need more points? Use 🛒 BUY POINTS button!",
        uid,
        c.message.message_id,
        parse_mode="Markdown"
    )
...
def get_points_button(m):
    ...
    message = f"💰 **Get More Points** 💰\n\n"
    message += f"✨ **Your Current Balance:** `{user.points()}` points\n\n"
    message += f"{purchase_msg}\n\n"
    message += f"🎁 **Ways to Earn Points:**\n"
    message += f"┌ 1. 🎁 **Referral System** - Share your link\n"
    message += f"├ 2. 🏆 **Redeem Codes** - Use coupon codes\n"
    message += f"├ 3. 📝 **Complete Tasks** - Check announcements\n"
    message += f"└ 4. 💎 **Direct Purchase** - Click 🛒 BUY POINTS\n\n"
    message += f"💎 **Click 🛒 BUY POINTS to see available packages!**\n\n"
...
def give_points_process(m):
    try:
        ...
        bot.send_message(m.from_user.id, 
            f"✅ **Points Added!**\n\n"
            f"👤 User: `{uid}`\n"
            f"💰 Old: `{old_points}` → New: `{user.points()}`\n"
            f"➕ Added: `+{pts}`",
            parse_mode="Markdown")
...
def fallback(m):
    ...
    known = [
        ...
        "📚 MY METHODS", "💎 GET POINTS", "🛒 BUY POINTS",
        "⚙️ ADMIN PANEL"
    ]
