# =========================
# ZEDOX BOT - COMPLETE VERSION WITH ENHANCEMENTS
# With: Subfolders, Fast Response, Fixed Give Points, Page Numbers, Auto Emojis, Category Counts, New Method Notifications
# =========================
...
# =========================
# 🚀 CACHE SYSTEMS (SPEED OPTIMIZATION)
# =========================
_cache_user = TTLCache(maxsize=2000, ttl=300)
_cache_folder = TTLCache(maxsize=1000, ttl=600)
_cache_config = TTLCache(maxsize=10, ttl=300)
_cache_force = TTLCache(maxsize=500, ttl=60)

def get_cached_config():
    key = "config"
    if key in _cache_config:
        return _cache_config[key]
    _cache_config[key] = get_config()
    return _cache_config[key]
...
def get_config():
    ...
    if not cfg:
        cfg = {
            ...
            "ref_reward": 5,
            "notify": True,
            "notify_new_methods": True,  # NEW: Toggle for new method notifications
...
# =========================
# 👤 USER SYSTEM (WITH CACHING)
# =========================
class User:
    @staticmethod
    def get(uid):
        uid = str(uid)
        if uid in _cache_user:
            return _cache_user[uid].copy()
    ...
    def __init__(self, uid):
        self.uid = str(uid)
        self.data = User.get(uid)
    
    def save(self):
        User.save(self.uid, self.data)
...
class FS:
    def add(self, cat, name, files, price, parent=None, number=None, text_content=None):
        ...
        # NEW: Send notification to all users if enabled
        cfg = get_cached_config()
        if cfg.get("notify_new_methods", True):
            self._notify_new_method(cat, name, number)
        
        clear_folder_cache()
        return number
    
    def _notify_new_method(self, cat, name, number):
        ...
        message += f"⬇️ <b>Use the buttons below to access!</b>"
        
        # Create inline keyboard with link to category
        ...
        if cat in cat_link_map:
            kb.add(InlineKeyboardButton(f"🔍 View {cat_link_map[cat]}", callback_data=f"goto_category|{cat}"))
        
        # Broadcast to all users
        def broadcast():
            for user in users_col.find({}, {"_id": 1}):
                try:
                    bot.send_message(int(user["_id"]), message, reply_markup=kb, parse_mode="HTML")
                    time.sleep(0.05)
                except:
                    pass
    ...
    def get(self, cat, parent=None):
        ...
        if cache_key in _cache_folder:
            return _cache_folder[cache_key]
        ...
        _cache_folder[cache_key] = result
        return result
...
# =========================
# 🏷️ AUTO EMOJI FUNCTION (NEW)
# =========================
def add_auto_emoji(name, cat=None):
    """Auto-add emojis to method names based on keywords"""
    emoji_map = {
        # Premium/Paid
        "premium": "💎", "pro": "⭐", "ultimate": "🔥", "advanced": "🚀", "exclusive": "✨",
        # Free/Basic
        "free": "🆓", "basic": "📌", "starter": "🌱", "beginner": "📘",
        # Apps
        "app": "📱", "android": "🤖", "ios": "🍎", "mobile": "📲", "iphone": "📱", "windows": "💻",
        # Services
        "service": "⚡", "api": "🔌", "tool": "🛠️", "software": "💿",
        # Methods
        "method": "🔧", "tutorial": "📚", "guide": "📖", "course": "🎓", "training": "🏋️",
        # Hacking/Cheats
        "hack": "🎮", "cheat": "🎯", "mod": "🔧", "crack": "🔓", "exploit": "⚡",
        # Keys/Licenses
        "key": "🔑", "license": "📜", "code": "💻", "serial": "🔢", "activation": "✅",
        # Accounts
        "account": "👤", "profile": "👥", "login": "🔐", "password": "🔒",
        # Payments
        "card": "💳", "bank": "🏦", "crypto": "🪙", "bitcoin": "₿", "usdt": "💵", "paypal": "💰",
        # Social Media
        "social": "📱", "instagram": "📸", "facebook": "📘", "twitter": "🐦", "tiktok": "🎵", "youtube": "📺",
        # Media
        "video": "🎥", "movie": "🍿", "series": "📺", "music": "🎵", "audio": "🎧",
        # Gaming
        "game": "🎮", "gaming": "🕹️", "pc": "💻", "console": "🎮", "xbox": "🎮", "playstation": "🎮",
        # Other
        "new": "🆕", "update": "🔄", "fix": "🔧", "patch": "📦"
    }
...
def force_block(uid):
    ...
    if cache_key in _cache_force:
        return _cache_force[cache_key]
    
    cfg = get_cached_config()
    force_channels = cfg.get("force_channels", [])
    
    if not force_channels:
        _cache_force[cache_key] = False
        return False
...
# =========================
# 📱 MAIN MENU (WITH CATEGORY COUNTS - NEW)
# =========================
def get_category_counts():
    """Get counts for each category with caching"""
    ...
    _cache_folder[cache_key] = counts
    return counts
...
def main_menu(uid):
    ...
    # Add categories with separate numbering and counts
...
def get_folders_kb(cat, parent=None, page=0, items_per_page=10):
    ...
    # PAGE NUMBER NAVIGATION - CLEARLY DISPLAYED
    nav_buttons = []
    if page > 0:
        nav_buttons.append(InlineKeyboardButton("◀️ PREV", callback_data=f"page|{cat}|{page-1}|{parent or ''}"))
    
    nav_buttons.append(InlineKeyboardButton(f"📄 PAGE {page+1} / {total_pages}", callback_data="noop"))
    
    if end < total_items:
        nav_buttons.append(InlineKeyboardButton("NEXT ▶️", callback_data=f"page|{cat}|{page+1}|{parent or ''}"))
...
def show_category(m):
    ...
    # Extract category from button text (remove count if present)
    text = m.text
    mapping = {
        "📂 FREE METHODS": "free",
        "💎 VIP METHODS": "vip",
    ...
    # Handle cases where button has count like "📂 FREE METHODS [5]"
    for key in mapping:
    ...
    if not data:
        bot.send_message(uid, f"📂 {m.text.split('[')[0].strip()}\n\nNo folders available!", parse_mode="Markdown")
        return
    
    bot.send_message(uid, f"📂 {m.text.split('[')[0].strip()}\n\nSelect:", reply_markup=get_folders_kb(cat))

# =========================
# 📂 OPEN FOLDER (WITH WORKING SUBFOLDERS & AUTO EMOJIS)
# =========================
...
def open_folder(c):
    ...
    if subfolders and len(subfolders) > 0:
        ...
        for sub in subfolders:
            ...
            display_name = add_auto_emoji(sub_name, cat)
            text = f"{icon} [{sub_number}] {display_name}"
            if sub_price > 0:
...
def buy_method(c):
    ...
    if user.purchase_method(method_name, price):
        ...
        bot.edit_message_text(
            f"✅ **Purchased!**\n\nYou now own: {add_auto_emoji(method_name, cat)}\nRemaining: {user.points()} pts",
            uid,
            c.message.message_id,
            parse_mode="Markdown"
        )
...
# =========================
# 🔔 NEW CATEGORY JUMP (for notifications)
# =========================
@bot.callback_query_handler(func=lambda c: c.data.startswith("goto_category|"))
def goto_category_callback(c):
    uid = c.from_user.id
    cat = c.data.split("|")[1]
    ...
    if cat not in cat_names:
        bot.answer_callback_query(c.id, "❌ Invalid category")
        return
    
    # Create fake message to reuse show_category
    fake_msg = type('obj', (object,), {'from_user': c.from_user, 'text': cat_names[cat]})
    show_category(fake_msg)
    bot.answer_callback_query(c.id)

# =========================
# CALLBACK HANDLERS (RESTORED - FULLY WORKING)
# =========================
...
def recheck(c):
    ...
    # Clear force cache for this user
    _cache_force.pop(f"force_{uid}", None)
...
def show_purchased_methods(m):
    ...
    for method in purchased:
        ...
        display_method = add_auto_emoji(method, "vip")
        kb.add(InlineKeyboardButton(f"[{number}] {display_method}", callback_data=f"open|vip|{method}|"))
...
def admin_menu():
    ...
    kb.row("🔔 Toggle Method Notify", "📊 Leaderboard")  # NEW: Toggle for method notifications
    kb.row("❌ Exit")
...
# =========================
# 🔔 TOGGLE METHOD NOTIFICATIONS (NEW)
# =========================
@bot.message_handler(func=lambda m: m.text == "🔔 Toggle Method Notify" and is_admin(m.from_user.id))
def toggle_method_notify(m):
    cfg = get_cached_config()
    current = cfg.get("notify_new_methods", True)
    set_config("notify_new_methods", not current)
    status = "ON" if not current else "OFF"
    bot.send_message(m.from_user.id, f"🔔 New method notifications: **{status}**\n\nUsers will {'now' if not current else 'no longer'} receive notifications when new methods are added.", reply_markup=admin_menu())
...
# =========================
# 📊 STATS (FIXED - CORRECT USER COUNT)
# =========================
...
def fallback(m):
    ...
    if m.text and m.text not in known and not m.text.startswith("📂") and not m.text.startswith("💎") and not m.text.startswith("📦") and not m.text.startswith("⚡"):
...
def run_bot():
    while True:
        try:
            print("=" * 60)
            print("🚀 ZEDOX BOT - ENHANCED VERSION 🚀")
            print(f"✅ Bot: @{bot.get_me().username}")
            print(f"👑 Owner: {ADMIN_ID}")
            print(f"💾 MongoDB: Connected")
            print(f"📁 Subfolders: WORKING")
            print(f"📊 Stats: FIXED (shows correct counts)")
            print(f"🏆 Leaderboard: WORKING")
            print(f"💰 Give Points: WORKING")
            print(f"⚡ Speed: OPTIMIZED with Caching")
            print(f"📄 Page Numbers: ENABLED (X/Y format)")
            print(f"🏷️ Auto Emojis: ENABLED")
            print(f"🔔 Method Notifications: {'ON' if get_config().get('notify_new_methods', True) else 'OFF'}")
