...
class FolderSystem:
    ...
    def delete(self, cat, name, parent=None):
        ...
        # Delete subfolders
        folders_col.delete_many({"category": cat, "parent": name})
        # Delete folder
    ...
    def generate(self, points, count, multi=False, days=None):
        ...
        for _ in range(count):
            ...
            codes_col.insert_one({
                ...
                "multi": multi, "used_count": 0, "max_uses": 10 if multi else 1,
                "expiry": expiry, "created": time.time(), "used_by": []
            })
            codes.append(code)
        return codes
...
@force_join
def cat_handler(m):
    titles = {"📂 FREE METHODS": "FREE", "💎 VIP METHODS": "VIP", "📱 PREMIUM APPS": "APPS", "⚡ SERVICES": "SERVICES"}
...
def open_cb(c):
    ...
    if not can and price > 0:
        ...
        kb.add(InlineKeyboardButton("⭐ VIP", callback_data="vip_info"))
...
@force_join
def redeem_cmd(m):
    msg = bot.send_message(m.from_user.id, "🎫 Enter code:", parse_mode="HTML")
...
def process_upload_price(m):
    ...
    try:
        ...
        kb.add("📝 Text", "📁 Files")
        kb.add("/cancel")
        msg = bot.send_message(uid, "Choose type:", reply_markup=kb, parse_mode="HTML")
...
# =========================
# 🗑️ DELETE FOLDER (FIXED)
# =========================
@bot.message_handler(func=lambda m: m.text == "🗑️ DELETE FOLDER" and is_admin(m.from_user.id))
def delete_cmd(m):
    msg = bot.send_message(m.from_user.id, "🗑️ Send: <code>category folder_name</code>\nExample: <code>free My Folder</code>", parse_mode="HTML")
...
def process_delete(m):
    ...
    bot.send_message(uid, f"⚠️ DELETE '{name}' from {cat.upper()}?\nAll subfolders will be deleted!", reply_markup=kb, parse_mode="HTML")
...
def confirm_del(c):
    ...
    if fs.delete(cat, name):
        ...
        bot.send_message(c.from_user.id, "✅ Deleted!", reply_markup=admin_kb())
    else:
...
def select_for_edit(m):
    ...
    bot.send_message(uid, f"📝 Edit: {name}\nWhat to edit?", reply_markup=kb, parse_mode="HTML")
...
def save_edit_text(m):
    ...
    s = edit_sessions[uid]
    folders_col.update_one({"category": s["cat"], "name": s["name"]}, {"$set": {"text_content": m.text}})
...
def process_edit_files(m):
    ...
    if m.text == "/done":
        ...
        folders_col.update_one({"category": s["cat"], "name": s["name"]}, {"$set": {"files": s["new_files"]}})
...
def process_move(m):
    ...
    if len(parts) != 2:
        ...
        return
    num, parent = int(parts[0]), parts[1] if parts[1] != "root" else None
    fs.move(num, parent)
    bot.send_message(m.from_user.id, f"✅ Folder #{num} moved!", reply_markup=admin_kb())
...
def process_vip(m, add):
    ...
    if add:
        if u.is_vip():
            bot.send_message(uid, "⚠️ Already VIP!")
            return
        u.make_vip()
        bot.send_message(uid, f"✅ User {target} is now VIP!")
        try:
            bot.send_message(target, "🎉 <b>You are now VIP!</b>\n\nAccess all VIP METHODS!", parse_mode="HTML")
        except:
            pass
    else:
        if not u.is_vip():
            bot.send_message(uid, "⚠️ Not VIP!")
            return
        u.remove_vip()
...
def process_give(m):
    ...
    try:
        ...
        if not user_data:
            bot.send_message(m.from_user.id, f"❌ User {target} not found! Must send /start first.")
            return
        ...
        bot.send_message(m.from_user.id, f"✅ Added +{points} pts to {target}\nOld: {old} → New: {u.get_points()}")
        
        try:
            bot.send_message(target, f"🎉 <b>+{points} points received!</b>\n\n💰 New balance: {u.get_points()} pts", parse_mode="HTML")
        except:
            pass
...
def gen_codes_cmd(m):
    ...
    bot.send_message(m.from_user.id, "🎫 Select code type:", reply_markup=kb, parse_mode="HTML")
...
def process_gen_count(m):
    ...
    try:
        ...
        if session["multi"]:
            ...
            bot.send_message(uid, f"✅ {count} Codes!\n\n<code>{chr(10).join(codes)}</code>", parse_mode="HTML", reply_markup=admin_kb())
...
def view_codes_cmd(m):
    codes = list(codes_col.find({}).sort("created", -1).limit(50))
    if not codes:
    ...
    try:
        ...
        if cmd == 'addadmin':
            if admins_col.find_one({"_id": uid}):
                ...
                return
            admins_col.insert_one({"_id": uid, "added": time.time()})
...
def set_contacts_cmd(m):
    ...
    kb.add(InlineKeyboardButton("📋 View", callback_data="view_contacts"))
    bot.send_message(m.from_user.id, "📞 Contact Settings:", reply_markup=kb)
...
def view_contacts_cb(c):
    ...
    bot.edit_message_text(f"📞 Points: {points}\n⭐ VIP: {vip}", c.from_user.id, c.message.message_id)
    bot.answer_callback_query(c.id)
...
def process_add_btn(m):
    ...
    if typ not in ["link", "folder"]:
        ...
        return
    if typ == "folder" and not fs.get_by_num(int(data)):
...
def process_add_ch(m):
    ...
    bot.send_message(m.from_user.id, f"✅ Added: {ch}", reply_markup=admin_kb())
...
def settings_cmd(m):
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(InlineKeyboardButton("⭐ VIP Msg", callback_data="set_vip_msg"))
    kb.add(InlineKeyboardButton("🏠 Welcome", callback_data="set_welcome"))
    kb.add(InlineKeyboardButton("💰 Ref Reward", callback_data="set_reward"))
    kb.add(InlineKeyboardButton("💵 Points/$", callback_data="set_ppd"))
...
def set_vip_msg_cb(c):
    ...
    bot.register_next_step_handler(msg, lambda x: update_config("vip_message", x.text) or bot.send_message(x.from_user.id, "✅ Updated!", reply_markup=admin_kb()))
    bot.answer_callback_query(c.id)
...
def set_welcome_cb(c):
    ...
    bot.register_next_step_handler(msg, lambda x: update_config("welcome_message", x.text) or bot.send_message(x.from_user.id, "✅ Updated!", reply_markup=admin_kb()))
    bot.answer_callback_query(c.id)
...
def set_reward_cb(c):
    ...
    bot.register_next_step_handler(msg, lambda x: update_config("referral_reward", int(x.text)) or bot.send_message(x.from_user.id, f"✅ Set to {x.text} points!", reply_markup=admin_kb()) if x.text.isdigit() else bot.send_message(x.from_user.id, "❌ Invalid!"))
    bot.answer_callback_query(c.id)
...
def stats_cmd(m):
    ...
    text = f"📊 <b>STATS</b>\n\n"
    text += f"👥 USERS: {total} (VIP: {vip})\n"
    text += f"💰 POINTS: {points:,} (Earned: {earned:,})\n"
    text += f"📚 CONTENT: FREE:{free_f} VIP:{vip_f} APPS:{apps_f} SVC:{svc_f}\n"
...
def broadcast_target_cb(c):
    target = c.data[3:]
    msg = bot.send_message(c.from_user.id, f"Send message to {target.upper()} users:", parse_mode="HTML")
...
def toggle_notify_cmd(m):
    ...
    bot.send_message(m.from_user.id, f"🔔 Notifications: {'ON' if not current else 'OFF'}", reply_markup=admin_kb())
...
def fallback(m):
    ...
    known = ["📂 FREE METHODS", "💎 VIP METHODS", "📱 PREMIUM APPS", "⚡ SERVICES", "💰 MY BALANCE", "⭐ VIP ACCESS", "🎁 REFERRAL", "👤 MY PROFILE", "🎫 REDEEM CODE", "💎 BUY POINTS", "⚙️ ADMIN PANEL"]
    if m.text not in known:
        bot.send_message(m.from_user.id, "❌ Use menu buttons", reply_markup=main_kb(m.from_user.id))
...
def run():
    while True:
        try:
            print("=" * 50)
            print("🚀 ZEDOX BOT - FAST & FIXED")
            print(f"✅ Bot: @{bot.get_me().username}")
            print(f"👑 Owner: {ADMIN_ID}")
            print(f"💾 MongoDB: Connected")
            print(f"⚡ Features: DELETE FOLDER ✓ | GIVE POINTS ✓ | CODES ✓")
            print("=" * 50)
