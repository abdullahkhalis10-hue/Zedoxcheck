...
# With Fixed Delete Folder & Optimized MongoDB Usage
# =========================

import telebot
from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton, ReplyKeyboardMarkup
import os, time, random, string, threading, hashlib, hmac, json, zlib
...
# =========================
# 🌐 MONGODB SETUP WITH OPTIMIZATION
# =========================
MONGO_URI = os.environ.get("MONGO_URI")
if not MONGO_URI:
    raise ValueError("MONGO_URI environment variable not set!")

client = MongoClient(MONGO_URI, maxPoolSize=10, compressors='snappy')
db = client["zedox_complete"]

# Enable compression for collections
db.command({"collMod": "users", "compression": "snappy"})
db.command({"collMod": "folders", "compression": "snappy"})
db.command({"collMod": "codes", "compression": "snappy"})

# Collections with indexes for better performance
users_col = db["users"]
...
users_col.create_index([("vip", 1)])
users_col.create_index([("points", -1)])
users_col.create_index([("refs", -1)])
folders_col.create_index([("cat", 1), ("parent", 1)])
folders_col.create_index([("number", 1)], unique=True)
codes_col.create_index([("used", 1)])
codes_col.create_index([("expiry", 1)])
...
# =========================
# 📦 DATA COMPRESSION FOR MONGODB
# =========================
def compress_data(data):
    """Compress data before storing in MongoDB"""
    json_str = json.dumps(data)
    compressed = zlib.compress(json_str.encode())
    return Binary(compressed)

def decompress_data(compressed_data):
    """Decompress data from MongoDB"""
...
class User:
    ...
    def save(self):
        # Update only changed fields to reduce MongoDB writes
...
# =========================
# 📦 CONTENT SYSTEM WITH FIXED DELETE
# =========================
class FS:
    ...
    def edit_name(self, cat, old, new, parent=None):
        ...
        # Update all subfolders' parent references
        folders_col.update_many(
            {"cat": cat, "parent": old},
            {"$set": {"parent": new}}
        )
...
def del_folder(m):
    try:
        ...
        bot.send_message(m.from_user.id, f"⚠️ **Confirm Deletion**\n\nAre you sure you want to delete '{name}' from {cat}?\n\nThis will also delete all subfolders inside it!", reply_markup=confirm_kb, parse_mode="Markdown")
...
def confirm_delete(c):
    ...
    if fs.delete(cat, name):
        bot.edit_message_text(f"✅ Deleted: {cat}/{name} and all its subfolders!", c.from_user.id, c.message.message_id, parse_mode="Markdown")
        bot.send_message(c.from_user.id, "✅ Folder deleted successfully!", reply_markup=admin_menu())
    else:
        bot.edit_message_text(f"❌ Folder '{name}' not found in {cat}!", c.from_user.id, c.message.message_id, parse_mode="Markdown")
...
def run_bot():
    while True:
        try:
            ...
            print(f"💾 Database: MongoDB (Optimized with compression)")
