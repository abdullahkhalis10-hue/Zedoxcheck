# =========================
# ZEDOX BOT - FAST OPTIMIZED VERSION
...
folder_cache = TTLCache(maxsize=200, ttl=120)
config_cache = TTLCache(maxsize=5, ttl=60)
...
# =========================
# 📁 FOLDER SYSTEM (WITH WORKING SUBFOLDERS & CACHE)
# =========================
class FS:
    def _get_cache_key(self, cat, parent):
        return f"folders_{cat}_{parent}"
    
    def _clear_cache(self, cat=None):
        if cat:
            keys_to_remove = [k for k in folder_cache.keys() if k.startswith(f"folders_{cat}_")]
            for k in keys_to_remove:
                folder_cache.pop(k, None)
        else:
            folder_cache.clear()
    ...
    def move_folder(self, number, new_parent):
        folder = folders_col.find_one({"number": number})
        if folder:
            folders_col.update_one({"number": number}, {"$set": {"parent": new_parent}})
            self._clear_cache(folder.get("cat"))
...
def run_bot():
    while True:
        try:
            print("=" * 50)
            print("🚀 ZEDOX BOT - FAST OPTIMIZED VERSION")
            ...
            print(f"⚡ Speed: OPTIMIZED (Caching + Threading)")
            print("=" * 50)
