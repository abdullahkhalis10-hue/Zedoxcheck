...
class FS:
    ...
    def get(self, cat, parent=None):
        """Get folders by category and parent - FIXED for subfolders"""
        query = {"cat": cat}
        if parent:
            query["parent"] = parent
        else:
...
def open_folder(c):
    ...
    # Check for subfolders - THIS IS THE KEY FIX
    subfolders = fs.get(cat, name)
    ...
    if files:
        ...
        bot.send_message(uid, "📁 No files in this folder.", parse_mode="Markdown")
