...
# With Fixed Points Giving, Multiple Admins, Contact Options
# ADDED: Delete Folder (Recursive), Edit Content, Text Methods
# =========================
...
# =========================
# 📦 CONTENT SYSTEM (UPDATED WITH DELETE RECURSIVE & EDIT)
# =========================
class FS:
    ...
    def delete_subfolders_recursive(self, cat, parent_name):
        """Delete all subfolders recursively"""
        subfolders = folders_col.find({"cat": cat, "parent": parent_name})
        for subfolder in subfolders:
            self.delete_subfolders_recursive(cat, subfolder["name"])
    ...
    def delete(self, cat, name, parent=None):
        ...
        if folder:
            ...
            # Delete all subfolders recursively
            self.delete_subfolders_recursive(cat, name)
    ...
    def edit_name(self, cat, old, new, parent=None):
        ...
        # Update subfolders parent references
    ...
    def edit_content(self, cat, name, content_type, content, parent=None):
        """Edit folder content (text or files)"""
        query = {"cat": cat, "name": name}
        if parent:
            query["parent"] = parent
    ...
    def get_content(self, cat, name, parent=None):
        """Get folder content"""
        query = {"cat": cat, "name": name}
        if parent:
            query["parent"] = parent
...
# =========================
# 💎 GET POINTS (WITH BUY OPTIONS INTEGRATED)
# =========================
...
def admin_menu():
    ...
    kb.row("📝 Edit Content", "🔀 Move Folder")  # NEW: Edit Content button
...
# =========================
# 🗑 DELETE FOLDER (UPDATED - WITH RECURSIVE DELETION)
# =========================
...
def del_folder(m):
    try:
        ...
        bot.send_message(m.from_user.id, f"⚠️ **Confirm Deletion**\n\nDelete '{name}' from {cat.upper()}?\n\nThis will also delete ALL subfolders inside it!", reply_markup=kb, parse_mode="Markdown")
...
def confirm_delete(c):
    ...
    if fs.delete(cat, name):
        bot.edit_message_text(f"✅ **Deleted Successfully!**\n\nRemoved: {cat.upper()} → {name} and all subfolders", c.from_user.id, c.message.message_id, parse_mode="Markdown")
        bot.send_message(c.from_user.id, "✅ Folder deleted!", reply_markup=admin_menu())
    else:
        bot.edit_message_text(f"❌ Folder not found!", c.from_user.id, c.message.message_id, parse_mode="Markdown")
...
def edit_text_callback(c):
    uid = c.from_user.id
    
    if uid not in edit_sessions:
        bot.answer_callback_query(c.id, "❌ Session expired! Please start over.")
        return
    
    session = edit_sessions[uid]
    session["step"] = "edit_text"
    
    # Get current content
    folder = fs.get_one(session["cat"], session["name"])
    ...
    msg = bot.send_message(
        uid,
        f"📝 **Edit Text Content**\n\n"
        f"📂 {session['cat'].upper()} → {session['name']}\n\n"
        f"**Current Content:**\n{current_preview}\n\n"
        f"Send the **NEW text content** for this folder:\n\n"
...
def save_edited_text(m):
    ...
    if m.text == "/cancel":
        ...
        bot.send_message(uid, "❌ Edit cancelled!", reply_markup=admin_menu())
        return
    
    if uid not in edit_sessions:
        bot.send_message(uid, "❌ Session expired! Please start over.", reply_markup=admin_menu())
        return
    ...
    # Update the content
    fs.edit_content(session["cat"], session["name"], "text", new_content)
...
def process_edit_files(m):
    ...
    if m.text == "/done":
        ...
        if not session.get("new_files"):
            bot.send_message(uid, "❌ No files uploaded! Send files or /cancel")
            return
        
        # Update the files
        fs.edit_content(session["cat"], session["name"], "files", session["new_files"])
    ...
    # Save file
    if m.content_type in ["document", "photo", "video"]:
        edit_sessions[uid]["new_files"].append({
            "chat": m.chat.id,
            "msg": m.message_id,
            "type": m.content_type
        })
        bot.send_message(uid, f"✅ File saved! ({len(edit_sessions[uid]['new_files'])} files)")
    else:
        bot.send_message(uid, "❌ Please send documents, photos, or videos only!")
    
    # Continue listening
    bot.register_next_step_handler(m, process_edit_files)

@bot.callback_query_handler(func=lambda c: c.data == "edit_cancel")
def edit_cancel_callback(c):
    uid = c.from_user.id
    
    edit_sessions.pop(uid, None)
    bot.edit_message_text("❌ Edit cancelled", uid, c.message.message_id)
    bot.send_message(uid, "Returning to admin panel...", reply_markup=admin_menu())
    bot.answer_callback_query(c.id)
...
def give_points_start(m):
    msg = bot.send_message(m.from_user.id, 
        ...
        "*User ID must be the Telegram Chat ID (found in 🆔 CHAT ID)*\n"
...
def give_points_process(m):
    try:
        ...
        if not user_data:
            bot.send_message(m.from_user.id, f"❌ User `{uid}` not found!\n\nMake sure the user has started the bot first by sending /start", parse_mode="Markdown")
            return
        ...
        try:
            ...
            bot.send_message(m.from_user.id, f"⚠️ Could not notify user `{uid}`. User may have blocked the bot.", parse_mode="Markdown")
            
    except ValueError:
        bot.send_message(m.from_user.id, "❌ Invalid points amount! Please enter a number.", parse_mode="Markdown")
    except Exception as e:
        bot.send_message(m.from_user.id, f"❌ Error: {str(e)}\n\nUse format: user_id points", parse_mode="Markdown")
...
def gen_codes_count(m, pts, multi_use):
    try:
        ...
        if count <= 0:
            ...
            return
        if count > 100:
            bot.send_message(m.from_user.id, "⚠️ Maximum 100 codes at once!")
            return
...
def run_bot():
    while True:
        try:
            ...
            print(f"💎 Buy Points: Integrated in GET POINTS")
            print(f"📞 Custom Contacts: ENABLED")
            print(f"🗑 Delete Folder: FIXED (Recursive)")
            print(f"📝 Edit Content: ADDED (Text & Files)")
            print("=" * 50)
