...
# With Working View Codes, Fast Response, All Features Working
# =========================
...
# =========================
# 🏆 CODES SYSTEM (WITH DELETE ALL FEATURE)
# =========================
class Codes:
    ...
    def delete_used_codes(self):
        result = codes_col.delete_many({"used": True})
        return result.deleted_count
    
    def delete_unused_codes(self):
        result = codes_col.delete_many({"used": False})
        return result.deleted_count
    
    def get_all_codes(self):
        return list(codes_col.find({}).sort("created_at", -1).limit(50))
...
# =========================
# 📂 OPEN FOLDER (WITH WORKING SUBFOLDERS & FIXED BUY BUTTON)
# =========================
...
def open_folder(c):
    ...
    # Check if user can access (VIP or already purchased)
    can_access = user.is_vip() or user.can_access_method(name)
    ...
    if text_content and not folder.get("files"):
        if not can_access:
            if price > 0:
                ...
                buy_kb.add(
                    InlineKeyboardButton(f"💰 Buy for {price} pts", callback_data=f"buy_{cat}_{name}_{price}"),
                ...
                bot.send_message(uid, f"🔒 **VIP Method: {name}**\n\nPrice: **{price} points**\nYour points: **{user.points()}**\n\nChoose an option:", reply_markup=buy_kb, parse_mode="Markdown")
            else:
                buy_kb = InlineKeyboardMarkup(row_width=2)
                buy_kb.add(
                    InlineKeyboardButton("⭐ BUY VIP", callback_data="get_vip"),
                ...
                bot.answer_callback_query(c.id, "🔒 This is a VIP method")
                bot.send_message(uid, f"🔒 **VIP Method: {name}**\n\nThis is a VIP method.\n\nChoose an option to access it:", reply_markup=buy_kb, parse_mode="Markdown")
            return
        
        # User has access, send content
    ...
    if not can_access and price > 0:
        if user.points() < price:
            ...
            buy_kb.add(InlineKeyboardButton("❌ Cancel", callback_data="cancel_buy"))
            bot.send_message(uid, f"🔒 **{name}**\n\nPrice: **{price} points**\nYour points: **{user.points()}**\n\nYou need more points!", reply_markup=buy_kb, parse_mode="Markdown")
            bot.answer_callback_query(c.id, f"❌ Need {price} points! You have {user.points()}", True)
            return
        user.spend_points(price)
        user.purchase_method(name, price)
    ...
    if files:
        ...
        if get_cached_config().get("notify", True):
            if count > 0:
                ...
                bot.send_message(uid, "❌ Failed to send files.")
    else:
        bot.send_message(uid, "📁 No files in this folder.")

# =========================
# 💰 BUY METHOD (FIXED - WORKING WITH UNDERSCORES)
# =========================
...
def buy_method(c):
    ...
    try:
        # Split the callback data: buy_category_name_price
        ...
        cat = parts[1]
        # Name can have underscores, so join everything between cat and price
        price_str = parts[-1]
        name = "_".join(parts[2:-1])
        
        price = int(price_str)
    except Exception as e:
        bot.answer_callback_query(c.id, f"❌ Error: {str(e)}")
        return
    
    if user.is_vip():
        bot.answer_callback_query(c.id, "✅ You are VIP! You have free access!", show_alert=True)
        # Re-open the folder to give access
    ...
    if user.can_access_method(name):
    ...
    if user.purchase_method(name, price):
        ...
        bot.edit_message_text(
            ...
            f"📦 **Item:** {name}\n"
            f"💰 **Points Spent:** {price}\n"
            f"💎 **Points Remaining:** {user.points()}\n\n"
            f"✨ Click the folder again to access your purchase!",
            uid,
            c.message.message_id,
            parse_mode="Markdown"
        )
        ...
        bot.answer_callback_query(c.id, "❌ Purchase failed! Please try again.", show_alert=True)
...
# =========================
# ⚙️ ADMIN PANEL (UPDATED WITH DELETE CODES BUTTON)
# =========================
def admin_menu():
    ...
    kb.row("📊 View Codes", "🗑 Delete Codes")
...
# =========================
# 📊 VIEW CODES (FIXED - WORKING)
# =========================
@bot.message_handler(func=lambda m: m.text == "📊 View Codes" and is_admin(m.from_user.id))
def view_codes(m):
    codes = codesys.get_all_codes()
    if not codes:
        bot.send_message(m.from_user.id, "📊 No codes generated yet!", parse_mode="Markdown")
        return
    
    total, used, unused, multi = codesys.get_stats()
    
    text = f"📊 **CODE STATISTICS**\n\n"
    text += f"┌ Total Codes: `{total}`\n"
    text += f"├ Used Codes: `{used}`\n"
    text += f"├ Unused Codes: `{unused}`\n"
    text += f"└ Multi-Use Codes: `{multi}`\n\n"
    
    # Show recent unused codes
    unused_codes = [c for c in codes if not c.get("used", False)][:10]
    if unused_codes:
        text += "**Recent Unused Codes:**\n"
        for code_data in unused_codes[:5]:
            text += f"• `{code_data['_id']}` - {code_data['points']} pts\n"
    
    bot.send_message(m.from_user.id, text, parse_mode="Markdown")

# =========================
# 🗑 DELETE CODES (NEW FEATURE)
# =========================
@bot.message_handler(func=lambda m: m.text == "🗑 Delete Codes" and is_admin(m.from_user.id))
def delete_codes_menu(m):
    total, used, unused, multi = codesys.get_stats()
    
    kb = InlineKeyboardMarkup(row_width=1)
    kb.add(
        InlineKeyboardButton(f"🗑 Delete ALL Codes ({total})", callback_data="delete_all_codes"),
        InlineKeyboardButton(f"🗑 Delete USED Codes ({used})", callback_data="delete_used_codes"),
        InlineKeyboardButton(f"🗑 Delete UNUSED Codes ({unused})", callback_data="delete_unused_codes"),
        InlineKeyboardButton("❌ Cancel", callback_data="cancel_delete_codes")
    )
    
    text = f"🗑 **DELETE CODES**\n\n"
    text += f"📊 Current Code Statistics:\n"
    text += f"┌ Total Codes: `{total}`\n"
    text += f"├ Used Codes: `{used}`\n"
    text += f"├ Unused Codes: `{unused}`\n"
    text += f"└ Multi-Use Codes: `{multi}`\n\n"
    text += f"⚠️ **Warning:** This action cannot be undone!"
    
    bot.send_message(m.from_user.id, text, reply_markup=kb, parse_mode="Markdown")

@bot.callback_query_handler(func=lambda c: c.data == "delete_all_codes")
def delete_all_codes_cb(c):
    if not is_admin(c.from_user.id):
        bot.answer_callback_query(c.id, "❌ Admin only!")
        return
    
    count = codesys.delete_all_codes()
    bot.answer_callback_query(c.id, f"✅ Deleted {count} codes!")
    bot.edit_message_text(f"✅ **Success!**\n\nDeleted all {count} codes from the database.", c.from_user.id, c.message.message_id)
    bot.send_message(c.from_user.id, "Returning to admin panel...", reply_markup=admin_menu())

@bot.callback_query_handler(func=lambda c: c.data == "delete_used_codes")
def delete_used_codes_cb(c):
    if not is_admin(c.from_user.id):
        bot.answer_callback_query(c.id, "❌ Admin only!")
        return
    
    count = codesys.delete_used_codes()
    bot.answer_callback_query(c.id, f"✅ Deleted {count} used codes!")
    bot.edit_message_text(f"✅ **Success!**\n\nDeleted {count} used codes from the database.", c.from_user.id, c.message.message_id)
    bot.send_message(c.from_user.id, "Returning to admin panel...", reply_markup=admin_menu())

@bot.callback_query_handler(func=lambda c: c.data == "delete_unused_codes")
def delete_unused_codes_cb(c):
    if not is_admin(c.from_user.id):
        bot.answer_callback_query(c.id, "❌ Admin only!")
        return
    
    count = codesys.delete_unused_codes()
    bot.answer_callback_query(c.id, f"✅ Deleted {count} unused codes!")
    bot.edit_message_text(f"✅ **Success!**\n\nDeleted {count} unused codes from the database.", c.from_user.id, c.message.message_id)
    bot.send_message(c.from_user.id, "Returning to admin panel...", reply_markup=admin_menu())

@bot.callback_query_handler(func=lambda c: c.data == "cancel_delete_codes")
def cancel_delete_codes_cb(c):
    bot.edit_message_text("❌ Cancelled", c.from_user.id, c.message.message_id)
    bot.send_message(c.from_user.id, "Returning to admin panel...", reply_markup=admin_menu())
    bot.answer_callback_query(c.id)
...
def run_bot():
    while True:
        try:
            ...
            print(f"💰 Buy Button: FIXED")
            print(f"📊 View Codes: FIXED")
            print(f"🗑 Delete Codes: ADDED")
