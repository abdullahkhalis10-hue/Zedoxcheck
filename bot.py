...
# =========================
class FS:
    def add(self, cat, name, files, price, parent=None, number=None):
        """Add folder with optional parent for nesting"""
        if number is None:
            config = get_config()
...
class Codes:
    ...
    def redeem(self, code, user):
        ...
        # Check max uses for multi-use codes
...
def start_cmd(m):
    ...
    # Strong referral system with validation
    if len(args) > 1:
        ref_id = args[1]
        ...
        if ref_id != str(uid) and ref_id.isdigit():
            ...
            # Check if current user hasn't been referred yet
            if ref_user_data and not user.data.get("ref"):
                # Verify referrer is not a bot or invalid
                try:
...
def open_folder(c):
    ...
    if subfolders:
        # This is a parent folder, show subfolders
...
def start_upload(uid, cat, is_service=False):
    ...
    bot.register_next_step_handler(msg, lambda m: upload_step(m, cat, uid, [], is_service))

def upload_step(m, cat, uid, files, is_service):
    if m.text == "/cancel":
        bot.send_message(uid, "❌ Upload cancelled", reply_markup=admin_menu())
        return
    
    if m.text == "/done":
        if not files and not is_service:
            bot.send_message(uid, "❌ No files uploaded!")
            return
        
        msg = bot.send_message(uid, "📝 **Folder name:**\n\nEnter the folder name:", parse_mode="Markdown")
        bot.register_next_step_handler(msg, lambda x: upload_name(x, cat, files, is_service))
        return
    ...
    bot.register_next_step_handler(m, lambda x: upload_step(x, cat, uid, files, is_service))

def upload_name(m, cat, files, is_service):
    name = m.text
    
    if is_service:
        msg = bot.send_message(m.from_user.id, "💰 **Price (points):**\n\nEnter 0 for free:", parse_mode="Markdown")
        bot.register_next_step_handler(msg, lambda x: service_price(x, cat, name))
    else:
        msg = bot.send_message(m.from_user.id, "💰 **Price (points):**\n\nEnter 0 for free:", parse_mode="Markdown")
        bot.register_next_step_handler(msg, lambda x: upload_save(x, cat, name, files))

def service_price(m, cat, name):
    price = int(m.text)
    msg = bot.send_message(m.from_user.id, "📝 **Service message:**\n\nMessage to send when user activates this service:", parse_mode="Markdown")
    bot.register_next_step_handler(msg, lambda x: service_save(x, cat, name, price))

def service_save(m, cat, name, price):
    number = fs.add(cat, name, [], price)
    folders_col.update_one(
        {"cat": cat, "name": name},
        {"$set": {"service_msg": m.text}}
    )
...
def upload_save(m, cat, name, files):
    try:
        ...
        bot.send_message(m.from_user.id, f"✅ **Upload successful!**\n\n📌 Folder number: `{number}`\n📂 Name: {name}\n💰 Price: {price} points\n📁 Files: {len(files)}", parse_mode="Markdown", reply_markup=admin_menu())
    except:
        bot.send_message(m.from_user.id, "❌ Invalid price! Please enter a number.", reply_markup=admin_menu())
...
def subfolder_parent(m, cat):
    ...
    bot.register_next_step_handler(msg, lambda x: subfolder_name(x, cat, parent))

def subfolder_name(m, cat, parent):
    name = m.text
    msg = bot.send_message(m.from_user.id, "💰 **Price (points):**\n\nEnter 0 for free:", parse_mode="Markdown")
    bot.register_next_step_handler(msg, lambda x: subfolder_price(x, cat, parent, name))

def subfolder_price(m, cat, parent, name):
    try:
        price = int(m.text)
        msg = bot.send_message(m.from_user.id, "📤 **Send files for this subfolder**\n\nSend files or press /done if no files:", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).row("/done", "/cancel"))
        bot.register_next_step_handler(msg, lambda x: subfolder_files(x, cat, parent, name, price, []))
    except:
        bot.send_message(m.from_user.id, "❌ Invalid price!")

def subfolder_files(m, cat, parent, name, price, files):
    if m.text == "/cancel":
    ...
    bot.register_next_step_handler(m, lambda x: subfolder_files(x, cat, parent, name, price, files))
...
def give_points_process(m):
    try:
        ...
        user = User(uid)
        user.add_points(int(pts))
        bot.send_message(m.from_user.id, f"✅ Added `{pts}` points to user `{uid}`!\n💰 New balance: `{user.points()}`", parse_mode="Markdown")
    except:
...
def set_points_start(m):
    msg = bot.send_message(m.from_user.id, "🔄 **Set Points**\n\nSend: user_id points\n\nExample: `123456789 500`", parse_mode="Markdown")
    bot.register_next_step_handler(msg, set_points_process)

def set_points_process(m):
    try:
        uid, pts = m.text.split()
        user = User(uid)
        user.set_points(int(pts))
        bot.send_message(m.from_user.id, f"✅ Set user `{uid}` points to `{pts}`!", parse_mode="Markdown")
    except:
        bot.send_message(m.from_user.id, "❌ Invalid format! Use: user_id points")
...
def run_bot():
    while True:
        try:
            ...
            print(f"📊 Features: Services, Nested Folders, Custom Buttons")
            print("=" * 50)
