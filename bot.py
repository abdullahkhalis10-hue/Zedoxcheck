...
# With Fixed Subfolders, Fixed Add Button, Strong Force Join
# =========================
...
# =========================
# 📁 FOLDER SYSTEM (WITH FIXED SUBFOLDERS)
# =========================
class FS:
...
# =========================
# 🚫 FORCE JOIN (STRONG - DETECTS WHEN USER LEAVES)
# =========================
_force_cache = {}
FORCE_CACHE_TTL = 10  # Reduced to 10 seconds for real-time checking

def force_block(uid):
    """Check if user joined required channels - STRONG CHECK"""
    global _force_cache
    now = time.time()
    ...
    # Always check fresh for non-admins (no cache for force join)
    for ch in force_channels:
        try:
            member = bot.get_chat_member(ch, uid)
            if member.status in ["left", "kicked"]:
                # User left or was kicked - show force join message
            ...
            # If can't check, show force join message
            kb = InlineKeyboardMarkup()
            for channel in force_channels:
                kb.add(InlineKeyboardButton(f"📢 Join {channel}", url=f"https://t.me/{channel.replace('@','')}"))
            kb.add(InlineKeyboardButton("✅ I Joined", callback_data="recheck"))
...
# =========================
# 📱 MAIN MENU (WITH WORKING CUSTOM BUTTONS)
# =========================
...
# =========================
# 📂 SHOW FOLDERS (WITH WORKING SUBFOLDERS)
# =========================
...
def open_folder(c):
    ...
    if subfolders:
        ...
        for sub in subfolders:
            ...
            sub_subfolders = fs.get(cat, sub_name)
            icon = "📁" if sub_subfolders else "📄"
...
def create_subfolder_process(m):
    try:
        ...
        if not parent_folder:
            bot.send_message(m.from_user.id, f"❌ Parent folder '{parent}' not found in {cat.upper()}!")
            return
...
def add_button_process(m):
    try:
        ...
        bot.send_message(m.from_user.id, f"✅ Button added: {btn_text}\n\nThe button will appear in the main menu!", reply_markup=admin_menu())
    except Exception as e:
        bot.send_message(m.from_user.id, f"❌ Error: {str(e)}\n\nUse format: type|text|data")
