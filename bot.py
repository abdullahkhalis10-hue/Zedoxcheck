...
# With Fixed Delete Folder (Recursive) & Optimized Speed
# =========================
...
class FS:
    ...
    def delete(self, cat, name, parent=None):
        ...
        # Update numbers for remaining folders
        if number:
...
def open_folder(c):
    ...
    for f in folder.get("files", []):
        try:
            ...
            count += 1
            time.sleep(0.2)  # Reduced from 0.3 to 0.2 for speed
        except:
            continue
...
def get_referral_callback(c):
    ...
    bot.edit_message_text(
        ...
        f"💰 For each friend who joins, you get `{get_cached_config().get('ref_reward', 5)}` points!\n\n"
...
# =========================
# 🗑 DELETE FOLDER - FULLY FIXED & WORKING
# =========================
...
def del_folder(m):
    try:
        ...
        # Show confirmation with folder details
        kb = InlineKeyboardMarkup()
        kb.add(
