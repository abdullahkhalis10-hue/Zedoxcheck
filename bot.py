...
# Separate numbering per category, Direct method buttons in notifications
# =========================
...
class User:
    def __init__(self, uid):
        ...
        if uid in _user_cache and (time.time() - _user_cache_time.get(uid, 0)) < 30:
            self.data = _user_cache[uid].copy()
            return
        ...
        if not data:
            data = {
                ...
                "hash_id": hash_user_data(self.uid),
    ...
    def save(self):
        ...
        _user_cache[self.uid] = self.data.copy()
...
# =========================
# 📁 FOLDER SYSTEM (SEPARATE NUMBERING PER CATEGORY)
# =========================
class FS:
    def _get_next_number(self, cat):
        """Get next number for specific category"""
        config = get_config()
        next_numbers = config.get("next_folder_number", {"free": 1, "vip": 1, "apps": 1, "services": 1})
    ...
    def add(self, cat, name, files, price, parent=None, number=None, text_content=None):
        if number is None:
            number = self._get_next_number(cat)
        ...
        # Send notification if enabled (with direct button)
        cfg = get_cached_config()
        if cfg.get("notify_new_methods", True):
            self._send_new_method_notification(cat, name, number)
        
        return number
    
    def _send_new_method_notification(self, cat, name, number):
        """Send notification with direct button to the method"""
        cat_names = {"free": "FREE", "vip": "VIP", "apps": "APPS", "services": "SERVICES"}
        cat_emoji = {"free": "📂", "vip": "💎", "apps": "📱", "services": "⚡"}
        
        message = f"🎉 **NEW METHOD ADDED!** 🎉\n\n"
        message += f"{cat_emoji.get(cat, '📌')} **Category:** {cat_names.get(cat, cat.upper())}\n"
        message += f"📌 **#{number}** {name}\n\n"
        message += f"✨ Click the button below to access it instantly!"
        
        # Create button that directly opens the method
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton(f"🔓 OPEN [{number}] {name}", callback_data=f"open|{cat}|{name}|"))
        
        # Broadcast to all users (async for speed)
        def broadcast():
            for user in users_col.find({}, {"_id": 1}):
                try:
                    bot.send_message(int(user["_id"]), message, reply_markup=kb, parse_mode="Markdown")
                    time.sleep(0.03)  # Very fast broadcast
                except:
                    pass
    ...
    def get(self, cat, parent=None):
        ...
        if parent:
            ...
            query["parent"] = None
        # Sort by number (now separate per category)
    ...
    def get_by_number_and_cat(self, cat, number):
        return folders_col.find_one({"cat": cat, "number": number})
...
def force_block(uid):
    ...
    if uid in _force_cache and (now - _force_cache[uid].get("time", 0)) < FORCE_CACHE_TTL:
...
def show_category(m):
    ...
    # Send typing indicator for faster feel
    ...
    # Send immediately with keyboard
    bot.send_message(uid, f"📂 {text}\n\nSelect:", reply_markup=get_folders_kb(cat))

# =========================
# 📂 OPEN FOLDER (ULTRA FAST)
# =========================
...
def open_folder(c):
    ...
    if not folder:
        bot.send_message(uid, "❌ Folder not found")
        return
    ...
    if text_content and not folder.get("files"):
        ...
        if cat != "vip" and price > 0 and not user.is_vip():
            if user.points() < price:
                bot.send_message(uid, f"❌ Need {price} pts! You have {user.points()}")
                return
            user.spend_points(price)
        
        bot.edit_message_text(f"📄 **{display_name}**\n\n{text_content}", uid, c.message.message_id, parse_mode="Markdown")
    ...
    if files:
        ...
        if get_cached_config().get("notify", True):
            bot.send_message(uid, f"✅ {display_name} sent!")
        bot.edit_message_text(f"✅ **{display_name}**\n\nContent sent!", uid, c.message.message_id, parse_mode="Markdown")
    else:
        bot.edit_message_text(f"📁 **{display_name}**\n\nNo files.", uid, c.message.message_id, parse_mode="Markdown")
...
def buy_method(c):
    ...
    if user.purchase_method(method_name, price):
        bot.send_message(uid, f"✅ Purchased! -{price} pts\n\nRemaining: {user.points()} pts")
        open_folder(c)
    else:
        bot.send_message(uid, "❌ Failed!")

# =========================
# CALLBACK HANDLERS (FAST)
# =========================
...
def buy_vip_points_callback(c):
    ...
    if user.points() >= vip_price_points:
        ...
        bot.send_message(uid, f"🎉 **CONGRATULATIONS!** 🎉\n\nYou are now VIP!\n\n💰 Points: {user.points()}", parse_mode="Markdown")
    else:
        bot.send_message(uid, f"❌ Need {vip_price_points} pts! You have {user.points()}", parse_mode="Markdown")
...
def recheck(c):
    ...
    if not force_block(uid):
        ...
        bot.send_message(uid, "❌ Join channels first!")
...
def admin_menu():
    ...
    kb.row("🔔 Toggle Method Notify", "📊 Leaderboard")
    kb.row("❌ Exit")
...
def give_points_process(m):
    try:
        ...
        if not user_id_str.isdigit():
            bot.send_message(m.from_user.id, f"❌ Invalid User ID! You sent: `{user_id_str}`", parse_mode="Markdown")
            return
        
        try:
            user_id = int(user_id_str)
        except ValueError:
            bot.send_message(m.from_user.id, f"❌ Invalid User ID!", parse_mode="Markdown")
            return
        
        try:
            points = int(points_str)
        except ValueError:
            bot.send_message(m.from_user.id, f"❌ Invalid Points!", parse_mode="Markdown")
            return
        
        if points <= 0:
            bot.send_message(m.from_user.id, "❌ Points must be greater than 0!", parse_mode="Markdown")
            return
        
        if points > 1000000:
            bot.send_message(m.from_user.id, "⚠️ Maximum 1,000,000 points per transaction!", parse_mode="Markdown")
            return
        
        user_data = users_col.find_one({"_id": str(user_id)})
        if not user_data:
            bot.send_message(m.from_user.id, f"❌ User `{user_id}` not found! Ask them to send /start first.", parse_mode="Markdown")
            return
        ...
        try:
            bot.send_message(user_id, f"🎉 **You received +{points:,} points!**\n\n💰 New balance: {new_points:,} 💎", parse_mode="Markdown")
        except:
            pass
            
    except Exception as e:
        bot.send_message(m.from_user.id, f"❌ Error! Use: `user_id points`", parse_mode="Markdown")
...
def run_bot():
    while True:
        try:
            ...
            print(f"🔔 Separate Category Numbering: ENABLED")
