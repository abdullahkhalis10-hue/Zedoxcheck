...
# With Fixed Buy Button & Delete All Codes Feature
# =========================
...
def buy_method(c):
    ...
    try:
        ...
        bot.answer_callback_query(c.id, f"Invalid purchase data: {str(e)}")
        return
    ...
    if user.purchase_method(method_name, price):
        ...
        # Also send a separate message with the content
        folder = fs.get_one(cat, method_name)
        if folder:
            text_content = folder.get("text_content")
            if text_content:
                bot.send_message(uid, f"📄 **{method_name}**\n\n{text_content}", parse_mode="Markdown")
            else:
                files = folder.get("files", [])
                if files:
...
def admin_menu():
    ...
    kb.row("📊 View Codes", "🗑 Delete Codes")  # NEW BUTTON
