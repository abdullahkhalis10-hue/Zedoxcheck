...
# With Services, Nested Folders, Dynamic Buttons, Points System
# =========================
...
# =========================
# 📱 MAIN MENU (WITHOUT SEPARATE BUY POINTS BUTTON)
# =========================
def main_menu(uid):
    ...
    kb.row("📚 MY METHODS", "💎 GET POINTS")  # BUY POINTS integrated here
...
def get_points_button(m):
    ...
    if active_packages:
        kb.add(InlineKeyboardButton("📞 Contact Admin to Buy Points", url=f"https://t.me/{admin_username}"))
...
def get_points_callback(c):
    ...
    # This is called from inline button, redirect to the get points function
    packages = get_points_packages()
    active_packages = [p for p in packages if p.get("active", True)]
...
def get_vip_info_callback(c):
    ...
    message += f"Click the button below to contact admin and become VIP today! 🚀"
    ...
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("📞 Contact Admin for VIP", url=f"https://t.me/{admin_username}"))
...
def subfolder_cat(m):
    ...
    msg = bot.send_message(m.from_user.id, "📁 **Parent folder name:**", parse_mode="Markdown")
...
def subfolder_parent(m, cat):
    ...
    if not parent_folder:
        bot.send_message(m.from_user.id, f"❌ Folder '{parent}' not found!")
        return
...
def subfolder_file_price(m, cat, parent, name):
    try:
        price = int(m.text)
        msg = bot.send_message(m.from_user.id, "📤 **Send files:**\n\nPress /done when finished", 
...
def move_folder_parent(m, number):
    ...
    bot.send_message(m.from_user.id, f"✅ Folder moved!", reply_markup=admin_menu())
...
def give_points_process(m):
    try:
        ...
        if not user_data:
            bot.send_message(m.from_user.id, f"❌ User `{uid}` not found in database!\n\nMake sure the user has started the bot first.", parse_mode="Markdown")
            return
...
def broadcast_target(c):
    ...
    msg = bot.send_message(c.from_user.id, "📝 **Send your broadcast message**", parse_mode="Markdown")
...
def add_button_type(c):
    ...
    if btn_type == "link":
        msg = bot.send_message(c.from_user.id, "🔗 **Button text:**", parse_mode="Markdown")
...
def button_link_text(m, btn_type):
    text = m.text
    msg = bot.send_message(m.from_user.id, "🔗 **Enter URL:**", parse_mode="Markdown")
...
def button_folder_text(m, btn_type):
    text = m.text
    msg = bot.send_message(m.from_user.id, "📂 **Enter folder number:**", parse_mode="Markdown")
...
def button_callback_text(m, btn_type):
    text = m.text
    msg = bot.send_message(m.from_user.id, "💬 **Enter callback data:**", parse_mode="Markdown")
...
def add_custom_link_start(m):
    msg = bot.send_message(m.from_user.id, "🔗 **Custom Link Button**\n\nEnter button text:", parse_mode="Markdown")
...
def run_bot():
    while True:
        try:
            ...
            print(f"📊 Features: Services, Nested Folders, Text Methods")
            print("=" * 50)
