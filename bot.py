...
# With Fixed VIP Count, Top Users Leaderboards, Improved Referral Message
# =========================
                ...
                try:
                    ...
                    try:
                        ...
                        bot.send_message(int(ref_id), 
                            ...
                            f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
                            f"✨ <b>@{user.username or user.uid}</b> just joined using your referral link!\n\n"
                            f"💰 <b>Reward Earned:</b> <code>+{reward} points</code>\n"
                            f"📊 <b>Total Referrals:</b> <code>{ref_user.get_refs_count()}</code>\n"
                            f"💎 <b>Total Points:</b> <code>{ref_user.points()}</code>\n"
                            f"🏆 <b>VIP Progress:</b> <code>{ref_user.get_refs_count()}/{get_cached_config().get('referral_vip_count', 50)}</code>\n\n"
                            f"━━━━━━━━━━━━━━━━━━━━━━\n"
...
def give_points_process(m):
    try:
        ...
        bot.send_message(m.from_user.id, f"✅ Added +{pts} pts to {uid}\nOld: {old} → New: {u.get_points()}")
        try:
            bot.send_message(int(uid), f"🎉 **+{pts} points received!**\n💰 New balance: {u.get_points()} pts", parse_mode="Markdown")
        except:
            pass
    except:
