# =========================
# ZEDOX BOT - RAILWAY OPTIMIZED VERSION
# Fastest Response, No Crashes, Working Subfolders
# =========================
...
# =========================
# 🌐 MONGODB SETUP (OPTIMIZED FOR RAILWAY)
# =========================
...
# Create indexes for speed (run once)
try:
...
def get_config():
    def fetch():
        ...
        return cfg
    return get_cached("config", fetch)
...
class User:
    def __init__(self, uid):
        ...
        if uid in _user_cache and (time.time() - _user_cache_time.get(uid, 0)) < 60:
...
class FS:
    def add(self, cat, name, files, price, parent=None, number=None, text_content=None):
        ...
        clear_cache(f"folders_{cat}_{parent}")
        return number
    
    def get(self, cat, parent=None):
        cache_key = f"folders_{cat}_{parent}"
        def fetch():
            query = {"cat": cat}
            if parent:
                query["parent"] = parent
    ...
    def move_folder(self, number, new_parent):
        ...
        if folder:
            ...
            clear_cache(f"folders_{folder.get('cat')}_")
...
# =========================
# 🚀 RUN BOT (RAILWAY OPTIMIZED)
# =========================
def run_bot():
    print("=" * 50)
    print("🚀 ZEDOX BOT - RAILWAY OPTIMIZED")
