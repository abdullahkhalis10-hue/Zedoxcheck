...
# With Working Subfolders, Fast Response, Original Design, Fixed VIP Stats
# =========================
    ...
    for ch in force_channels:
        try:
            ...
            if member.status in ["left", "kicked"]:
                ...
                bot.send_message(uid, "🚫 **Access Restricted!**\n\nPlease join the following channels to use this bot:", reply_markup=kb, parse_mode="Markdown")
                return True
        except:
            kb = InlineKeyboardMarkup()
            for channel in force_channels:
                kb.add(InlineKeyboardButton(f"📢 Join {channel}", url=f"https://t.me/{channel.replace('@','')}"))
            kb.add(InlineKeyboardButton("✅ I Joined", callback_data="recheck"))
            bot.send_message(uid, f"🚫 **Please join the required channels first!**", reply_markup=kb, parse_mode="Markdown")
            return True
...
def get_points_button(m):
    ...
    if active_packages:
        ...
        message += f"💳 **Accepted Payment Methods:**\n"
...
# =========================
# 📂 OPEN FOLDER (WITH WORKING SUBFOLDERS - ORIGINAL DESIGN)
# =========================
    ...
    if subfolders:
        ...
        for sub in subfolders:
            ...
            keyboard.add(InlineKeyboardButton(
                text,
                callback_data=f"open|{cat}|{sub_name}|{name}"
            ))
        
        keyboard.add(InlineKeyboardButton("🔙 BACK", callback_data=f"back|{cat}|{name}"))
...
# =========================
# CALLBACK HANDLERS (ORIGINAL DESIGN)
# =========================
...
def admin_menu():
    ...
    kb.row("🏦 Binance Settings", "📸 Screenshot Setting")
...
def upload_type_choice(m, cat, is_service):
    if m.text == "/cancel":
        if m.from_user.id in upload_sessions:
...
# =========================
# 📁 CREATE SUBFOLDER (ORIGINAL DESIGN)
# =========================
...
# =========================
# 🗑 DELETE FOLDER (ORIGINAL DESIGN - WORKING)
# =========================
...
# =========================
# 🏆 GENERATE CODES (ORIGINAL DESIGN)
# =========================
...
def gen_codes_type(c):
    ...
    bot.code_gen_session[c.from_user.id] = {"multi_use": multi_use, "step": "points"}
...
def process_code_points(message):
    ...
    try:
        ...
        bot.send_message(user_id, "❌ Invalid points! Enter a number.")
...
def process_code_count(message):
    ...
    try:
        ...
        if session["multi_use"]:
            ...
            codes = codesys.generate(session["points"], count, session["multi_use"])
            codes_text = "\n".join(codes)
        ...
        bot.send_message(user_id, "❌ Invalid count! Enter a number.")
...
def process_code_expiry(message, points, count, multi_use):
    ...
    try:
        ...
        codes = codesys.generate(points, count, multi_use, expiry_days)
        codes_text = "\n".join(codes)
...
# =========================
# 📊 STATISTICS (FIXED - SHOWS VIP COUNT)
# =========================
...
def show_stats(m):
    ...
    total = len(users)
    vip = sum(1 for u in users if u.get("vip", False) == True)
    free = total - vip
    ...
    total_codes, used_codes, unused_codes, multi_codes = codesys.get_stats()
    
    stats_msg = f"📊 **ZEDOX STATISTICS**\n\n"
    stats_msg += f"👥 **USERS:**\n"
    stats_msg += f"┌ Total Users: `{total}`\n"
    stats_msg += f"├ VIP Users: `{vip}`\n"
    stats_msg += f"└ Free Users: `{free}`\n\n"
    ...
    stats_msg += f"📚 **CONTENT:**\n"
    stats_msg += f"┌ FREE METHODS: `{free_folders}`\n"
    stats_msg += f"├ VIP METHODS: `{vip_folders}`\n"
    stats_msg += f"├ PREMIUM APPS: `{apps_folders}`\n"
    stats_msg += f"└ SERVICES: `{services_folders}`\n\n"
    
    stats_msg += f"📈 **ACTIVITY:**\n"
    stats_msg += f"┌ Total Referrals: `{total_refs}`\n"
    stats_msg += f"├ Total Purchases: `{total_purchases}`\n"
    stats_msg += f"├ Total Codes: `{total_codes}`\n"
    stats_msg += f"├ Used Codes: `{used_codes}`\n"
    stats_msg += f"└ Unused Codes: `{unused_codes}`"
    
    bot.send_message(m.from_user.id, stats_msg, parse_mode="Markdown")

# =========================
# 📦 POINTS PACKAGES MANAGEMENT (ORIGINAL DESIGN)
# =========================
...
# =========================
# 💰 SET POINT PRICE (ORIGINAL DESIGN)
# =========================
...
# =========================
# 🔘 CUSTOM BUTTONS (ORIGINAL DESIGN)
# =========================
...
# =========================
# 🔗 ADD CUSTOM LINK (ORIGINAL DESIGN)
# =========================
...
# =========================
# ➕ ADD FORCE JOIN (ORIGINAL DESIGN)
# =========================
...
# =========================
# ⚙️ VIP SETTINGS (ORIGINAL DESIGN)
# =========================
...
# =========================
# 💳 PAYMENT METHODS (ORIGINAL DESIGN)
# =========================
...
# =========================
# 👥 ADMIN MANAGEMENT (ORIGINAL DESIGN)
# =========================
...
# =========================
# 🏦 BINANCE SETTINGS (ORIGINAL DESIGN)
# =========================
...
# =========================
# 📸 SCREENSHOT SETTING (ORIGINAL DESIGN)
# =========================
...
def run_bot():
    while True:
        try:
            ...
            print(f"🚫 Force Join: STRONG (detects leaves)")
            print(f"📁 Subfolders: WORKING")
            print(f"➕ Add Button: WORKING")
            print(f"📊 VIP Count: SHOWING CORRECTLY")
            print("=" * 50)
