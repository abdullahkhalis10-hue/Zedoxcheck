# =========================
# ZEDOX BOT - COMPLETE ORIGINAL VERSION (FIXED & OPTIMIZED)
# With Working Subfolders, Fast Response, Fixed Give Points
# ALL ORIGINAL FEATURES PRESERVED - NO CRASHES
# =========================
...
# =========================
# 🚀 SIMPLE CACHE (NO EXTRA DEPENDENCIES)
# =========================
_config_cache = None
...
def get_config():
    ...
    if not cfg:
        cfg = {
            ...
            "ref_reward": 5,
            "notify": True,
            "notify_new_methods": True,  # NEW: Toggle for method notifications
...
class FS:
    def add(self, cat, name, files, price, parent=None, number=None, text_content=None):
        ...
        # NEW: Send notification if enabled
    ...
    def _send_new_method_notification(self, cat, name, number):
        ...
        def broadcast():
            for user in users_col.find({}, {"_id": 1}):
                try:
                    bot.send_message(int(user["_id"]), message, parse_mode="Markdown")
                    time.sleep(0.05)
                except:
                    pass
...
def force_block(uid):
    ...
    for uid_key in list(_force_cache.keys()):
        if now - _force_cache[uid_key].get("time", 0) > FORCE_CACHE_TTL:
            del _force_cache[uid_key]
...
# =========================
# 📂 SHOW FOLDERS (WITH PAGE NUMBERS - NEW)
# =========================
...
def show_category(m):
    ...
    # Handle buttons with counts like "📂 FREE METHODS [5]"
...
# =========================
# CALLBACK HANDLERS (ALL ORIGINAL)
# =========================
...
def give_points_process(m):
    try:
        ...
        bot.send_message(m.from_user.id, f"❌ **Error!** Use: `user_id points`\nError: {str(e)}", parse_mode="Markdown")
